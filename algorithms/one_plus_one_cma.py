...
class OnePlusOneCMABase(Algorithm):
    ...
    def initialize(self, x0: np.ndarray, sigma0: float):
        ...
        # Placeholder matrix, for CMA this is C for C-CMA this is A_inv
    ...
    def update_strategy(self):
        if self.p_succ < self.p_thres:
            ...
            self.C = (1 - self.ccov) * self.C + (
                self.ccov * np.dot(self.pc, self.pc.T)
            )
        else:
            self.pc = (1 - self.cc) * self.pc
            self.C = ((1 - self.ccov) * self.C) + (
                self.ccov * (
                    np.dot(self.pc, self.pc.T) + (self.cc2 * self.C)
                )
            )
...
class OnePlusOneCholeskyCMAES(OnePlusOneCMABase):
    ...
    @A_inv.setter
    def A_inv(self, A_inv: np.ndarray):
        self.M = A_inv

    def update_strategy(self):
        if self.p_succ < self.p_thres:
            self.pc = ((1 - self.cc) * self.pc) + (
                np.sqrt(self.cc2) * self.y
            )
            alpha = 1 - self.ccov
        else:
            self.pc = (1 - self.cc) * self.pc
        ...
        self.A_inv = (1 / sqrt_a) * self.A_inv - (
            (1 / (sqrt_a * w_norm)) * (
                (1 - (1 / np.sqrt(1 + (b_a * w_norm)))) * np.dot(w, np.dot(w.T, self.A_inv))   
            )
        )        